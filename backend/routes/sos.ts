...
sosRouter.get("/:id", async (req: Request, res: Response) => {
  ...
  const sosQuery = {
    "gaurd.securityCompany": ValidObjectId(id),
  };
  try {
...
sosRouter.delete("/:id", async (req: Request, res: Response) => {
  const id = getRequestIdParam(req);

  try {
    const deletedSOS = await sosSchema.findByIdAndDelete(ValidObjectId(id)).exec();
